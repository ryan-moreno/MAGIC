...
def write_target_data(summary_df, target_data, padj_cutoff, folder, region_colname):

    # target_data = dict of dfs.
    # key =expt, val = target_df with cols [[region_colname,"ChIP"]]
    ...
    for e in expts:
        ...
        _df.columns = [region_colname, "ChIP"]
...
####################################################################################
def write_gmx_file(
    summary_df, master_ls, target_data, padj_cutoff, f_path, region_colname
):
    ...
    for expt in experiments:
        tf = expt.split(":")[1]
        genes = target_data[expt][region_colname].to_list()
